...
import Image from "next/image"; // Use Next.js Image for all images
import Tabs from "@mui/material/Tabs";
import Tab from "@mui/material/Tab";
import { fetchDataFromApi, postData } from "@/utils/api";
import { MyContext } from "@/context/ThemeProvider";
...
// Dynamically import heavy components with suspense and SSR disabled
const ProductsSlider = dynamic(() => import("@/components/ProductsSlider"), { ssr: false, suspense: true });
const AdsBannerSliderV2 = dynamic(() => import("@/components/AdsBannerSliderV2"), { ssr: false, suspense: true });
const AdsBannerSlider = dynamic(() => import("@/components/AdsBannerSlider"), { ssr: false, suspense: true });
const BannerBoxV2 = dynamic(() => import("@/components/bannerBoxV2"), { ssr: false, suspense: true });
const HomeBannerV2 = dynamic(() => import("@/components/HomeSliderV2"), { ssr: false, suspense: true });
const BlogItem = dynamic(() => import("@/components/BlogItem"), { ssr: false, suspense: true });
const HomeSlider = dynamic(() => import("../HomeSlider"), { ssr: false, suspense: true });

export default function Home({ initialHomeSlides = [], initialCatData = [], initialSiteSettings = {}, initialFreeDeliveryFee = 0 }) {
  // Use initial SSR/SSG props for above-the-fold content
  const { convertPrice, getSymbol } = useCurrency();
  const [value, setValue] = useState(0);

  // All sections data
  const [homeSlidesData, setHomeSlidesData] = useState(initialHomeSlides);
  const [popularProductsData, setPopularProductsData] = useState([]);
  const [productsData, setAllProductsData] = useState([]);
  const [featuredProducts, setFeaturedProducts] = useState([]);
  const [bannerV1Data, setBannerV1Data] = useState([]);
  ...
  const [blogData, setBlogData] = useState([]);
  const [randomCatProducts, setRandomCatProducts] = useState([]);
  const [freeDeliveryFee, setFreeDeliveryFee] = useState(initialFreeDeliveryFee);
  ...
  // Data fetching - only for non-above-the-fold content or after hydration
  const loadHeroData = useCallback(async () => {
    if (loadedSections.has('hero')) return;
    setLoadingState('hero', true);
    markSectionLoaded('hero');
  ...
  const loadBulkHomeProducts = useCallback(async () => {
    ...
    try {
      ...
      const res = await postData("/api/product/getBulkProductsHome", {
        popularCatId, randomCatIds, featured: true, latest: true,
      });
    ...
    } catch (err) { setError("Failed to load homepage products."); }
    setLoadingState('popular', false);
    setLoadingState('featured', false);
    setLoadingState('randomCat', false);
    setLoadingState('latest', false);
  ...
  // Intersection observers for lazy section loading
  useEffect(() => {
    ...
    const sectionsConfig = [
      { ref: heroRef, callback: loadHeroData },
      { ref: popularRef, callback: () => { loadSiteSettings(); loadBulkHomeProducts(); } },
      { ref: latestRef, callback: () => {} }, // bulk
      { ref: featuredRef, callback: () => {} }, // bulk
      { ref: bannerV1Ref, callback: loadBannerV1Data },
      { ref: bannerList2Ref, callback: loadBannerList2Data },
      { ref: randomCatRef, callback: () => {} }, // bulk
      { ref: blogRef, callback: loadBlogData },
      { ref: shippingRef, callback: loadShippingData },
    ];
    sectionsConfig.forEach(({ ref, callback }) => {
  ...
  return (
    <>
      {/* HERO SECTION SSR/SSG OPTIMIZED */}
      <div ref={heroRef}>
        {(!loadedSections.has('hero') && !homeSlidesData.length) ? (
          <div className="h-[400px] bg-gray-100 animate-pulse"></div>
        ) : (
        ...
        {loadedSections.has('blog') && blogData?.length > 0 && (
          <section className="py-5 pb-8 pt-0 bg-white blogSection">
            <div className="container">
              ...
              {loadingStates.blog && <div className="h-[200px] bg-gray-50 animate-pulse rounded"></div>}
              <Suspense fallback={<div className="h-[200px] bg-gray-50 animate-pulse rounded"></div>}>
                {blogData?.length > 0 && (
                  <Swiper
                    slidesPerView={4}
                    spaceBetween={30}
